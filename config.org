...
Sometimes elpaca reports an error for some packages on first load: "Cannot determine url from recipe". The solution to this problem is just to delete the elpaca cache files in =~/emacs-conf/elpaca/cache= and restart emacs.
...
If I'm on linux and I have proper window tiling, get a bit fancy and put window transparency on.
...
Now we set up the actual capture templates. Here the capture-target set in the wrapper function above is used as the target file, and the headline depends on the type of capture.
...
=highlight-indent-guides= provides bars to show how many levels of indentation down a particular block of code is. We only want to turn it on for =prog-mode= buffers, so hook into the minor mode after =prog-mode= is started.
...
For navigating around the file system, we could use =dired=. However, let's try =dirvish= which tries to provide a more batteries-included experience.

I'll come back to this to try customising it some more.
